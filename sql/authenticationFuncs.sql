...
create view users_view as select users.name,users.username,users.password from users;
create or replace rule ins_user as on insert to users_view
	do instead insert into users(name,username,enc_pass,salt)
	values (new.name,new.username,make_pass(make_salt(new.password)||new.password),make_salt(new.password));
